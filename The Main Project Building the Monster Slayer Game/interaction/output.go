...
func ShowAvailableActions(specialAttackIsAvailabele bool) {
	fmt.Println("Please choose your action")
	fmt.Println("--------------------------")
	fmt.Println("(1) Attack Monster")
	fmt.Println("(2) Heal")

	if specialAttackIsAvailabele {
...
func PrintRoundStatistics(roundData *RoundData) {
	if roundData.Action == "ATTACK" {
		...
		fmt.Printf("Player performed a strong attack again monster for %v damage.\n", roundData.PlayerAttackDmg)
	} else {
...
func WriteLogFile(rounds *[]RoundData) {
	...
	for index, value := range *rounds {
		...
		if err != nil {
			fmt.Println("Writing into log file.Existing. ")
			continue
		}
	}
	file.Close()
